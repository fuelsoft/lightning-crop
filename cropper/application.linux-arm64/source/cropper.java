...
public class cropper extends PApplet {
...
public void filepicked(File selection){
  noLoop();
  if(selection == null) {
    loop();
    return;
...
public void shading(int x1, int y1, int x2, int y2) {
  rectMode(CORNERS);
  fill(0, 0, 0, 127);
  rect(x1, y1, x2, y2);
}

public int[] setCorners(int x1, int y1, int x2, int y2) {
...
public PImage imageScale(PImage reference, float factor){
...
public void mousePressed() { //mouse clicked, check and trigger save
  if (mouseButton == LEFT){
...
public void keyPressed() {
  ...
  if (key == '-') { //scale window down (zoom out)
    if (width >= 150) {
      ...
      newBG = imageScale(bg, 1/1.1f);
      surface.setSize(newBG.width, newBG.height);
    }
  }
}

public void sysprint(String type, String msg) {
  System.err.println(type+": "+msg);
}
  public void settings() {  size(480, 360); }
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "cropper" };
    if (passedArgs != null) {
